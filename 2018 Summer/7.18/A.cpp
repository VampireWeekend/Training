...
int l[maxn],r[maxn],q[maxn],f[maxn],seq[maxn];
...
int main() {
	...
	sort(b+1,b+n+1);
	for (i=1;i<=n;i++) seq[b[i].second]=i;
	int root=build(1,n);
	for (i=1;i<=n;i++) {
	...
	for (i=1;i<=c;i++) {
		int size=vi[i].size();
		for (j=0;j<size;j++) 
			if (!z[vi[i][j]]) dfs3(vi[i][j]);
	}
	for (i=1;i<=n;i++) 
		if (!z[f[i]]) s[f[i]].insert(pp(a[b[i].second].c,b[i].second));
	ll ans=0;
	for (i=1;i<=c;i++) 
	...
	for (i=1;i<=qn;i++) {
		...
		s[f[x]].erase(pp(a[b[x].second].c,x));
		a[b[x].second].c=y;
		s[f[x]].insert(pp(y,x));
		ans+=(ll)s[f[x]].begin()->first;
		printf("%lld\n",ans);
